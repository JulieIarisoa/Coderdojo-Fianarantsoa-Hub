...
export async function syncUserProfile(
  ...
  try {
    ...
    if (userSnap.exists()) {
      return { ...fallbackProfile, ...(userSnap.data() as UserProfile), id: user.uid };
    }
    ...
    console.warn("Firestore profile sync warning (using Auth fallback profile):", err);
    return fallbackProfile;
  }
}
    ...
    async (firebaseUser) => {
      if (firebaseUser) {
        try {
          ...
          console.warn("Auth state change sync failed, using fallback:", err);
          callback({
            id: firebaseUser.uid,
